...
def objective(trial):
    ...
    config = [
        ...
        "--hidden-dim", "256",
        "--seed", "1",
        "--exp-name", f"optuna_scaled/softmod/trial_{trial.number}",
        "--eval-freq", "50000"
    ]
    
    # Soft Mod Specifics
    config.extend(["--num-modules", str(params["num_modules"])])
    config.extend(["--num-layers", str(params["num_layers"])])
    ...
    try:
        ...
        save_dir = os.path.join(base_dir, f"optuna_scaled/softmod/trial_{trial.number}")
...
if __name__ == "__main__":
    ...
    study = optuna.create_study(
        study_name="mt10_softmod_scaled",
        storage=storage,
        direction="maximize",
        load_if_exists=True,
