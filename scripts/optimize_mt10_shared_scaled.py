...
def objective(trial):
    ...
    # 1. Sample Hyperparams (Shared uses standard params)
    ...
    # Check if get_trial_params gives embedding dim? usually not.
    # We defaults to task-embedding enabled in train_baseline_ppo for 'shared'?
    # Actually 'shared' typically implies Multi-Task PPO on shared backbone.
    # We should ensure --use-task-embedding is on if the baseline supports it?
    # Actually standard shared usually concatenates one-hot.
    ...
    try:
        ...
        save_dir = os.path.join(base_dir, f"optuna_scaled/shared/trial_{trial.number}")
