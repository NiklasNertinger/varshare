...
At its core, VarShare was designed to solve negative transfer and model bloat in Multi-Task Reinforcement Learning (MTRL). The original approach calculated effective task weights $w_t$ as the sum of a deterministic shared backbone $\theta$ and probabilistic task-specific residuals $\mu_t$, sampled from a learned distribution $\mathcal{N}(\mu_t, \sigma_t^2)$. The architecture relied on two core concepts:
\begin{enumerate}
    \item \textbf{Deterministic Shared Base:} A rigid, shared backbone ($\theta$) optimized across all tasks to extract general, universal features.
    \item \textbf{Variational Task-Specific Residuals:} Probabilistic distribution shifts ($\sim \mathcal{N}(\mu_t, \sigma_t^2)$) learned exclusively for each task, allowing the model to safely adapt independent task logic.
\end{enumerate}
