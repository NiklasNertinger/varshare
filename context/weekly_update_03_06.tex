...
\begin{itemize}
    ...
    \item \textbf{Soft Muting / Selective Gating:} To solve "Feature Interference", we are experimenting with a task-specific vector of learned gates $g_t$. Instead of harsh boolean masking, these gates are passed through a Sigmoid function so their values are strictly continuous between 0 and 1. The forward pass becomes: $y_{task} = (\text{sigmoid}(g_t) \odot \theta + \mu_t)x$. If a task fundamentally disagrees with a specific shared feature in $\theta$, it can simply map that specific gate $g_{t,i}$ towards $0.0$. By multiplying the conflicting feature by zero, it entirely silences it \textit{without} forcing the network to learn a massive, heavily regularized negative number in $\mu_t$ to manually subtract it, perfectly avoiding the L2 penalty and disentangling the task.
\end{itemize}
